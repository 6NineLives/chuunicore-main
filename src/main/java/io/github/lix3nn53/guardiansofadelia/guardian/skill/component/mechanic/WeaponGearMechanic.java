...
public class WeaponGearMechanic extends MechanicComponent {
    ...
    public WeaponGearMechanic(String rpgClass, ConfigurationSection configurationSection, int skillId) {
        ...
        GearMechanicSkillManager.register(rpgClass, weaponGearType, skillId);
    }
    ...
    public boolean execute(LivingEntity caster, int skillLevel, List<LivingEntity> targets, int castCounter, int skillId) {
        ...
        for (LivingEntity target : targets) {
            if (target instanceof Player player) {
                player.sendMessage(ChatPalette.YELLOW + "You have unlocked a new weapon: " + ChatPalette.BLUE_LIGHT + weaponGearType.toString());
            }
        }
